"""GNN-TCN model for economic impact prediction.

Architecture identical to the Donbas model (polymarket/network/gnn/model.py),
adapted for West Africa economic network with quarterly resolution.

Forward pass:
  1. Input: (batch, N_cities, window_size, 12_features)
  ...
  4. Prediction head: FC -> output per FTZ target city -> (batch, n_targets)
  5. Platt scaling (post-hoc): calibrates outputs

Dilated TCN layers: receptive field grows exponentially without pooling,
layer 1 sees every step, top layer sees every 4th step via dilation [1, 2, 4, 8].
"""
...
class TCN(nn.Module):
    ...
    With 4 layers and kernel=3, receptive field = 2 * (3-1) * (1+2+4+8) + 1 = 61.
    For 16-quarter window this provides full coverage.
    """
...
class GraphAttention(nn.Module):
    ...
    Uses dense attention over the adjacency matrix -- feasible for 45-node graph.
    """
    ...
    def forward(self, x: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        ...
        # Aggregate
        # attn: (B, N, H, N) x h.permute: (B, N_src, H, D) -> need (B, N, H, D)
...
class GNNTCN(nn.Module):
    """Graph Attention + Temporal Convolution Network for economic prediction.

    Forward pass:
      1. For each timestep t in [0, W): GAT encodes spatial features
      2. Per-node temporal sequence fed through TCN
      3. Only FTZ target cities get prediction heads
      4. Output: raw logits (apply sigmoid + Platt scaling externally)
    """

    def __init__(self, cfg: Optional[ModelConfig] = None, n_nodes: int = 45, target_indices: Optional[list[int]] = None):
        super().__init__()
        self.cfg = cfg or ModelConfig()
        self.n_nodes = n_nodes
        self.target_indices = target_indices or list(range(self.cfg.n_targets))
    ...
    def forward(
        ...
        """
        Args:
            x: (batch, N_cities, window_size, n_features) -- temporal graph snapshots
            adj: (N_cities, N_cities) -- adjacency matrix with self-loops

        Returns:
            logits: (batch, n_targets) -- raw logits for FTZ target cities
        """
        B, N, W, F = x.shape
...
class PlattScaling(nn.Module):
    ...
    For FTZ analysis: calibrated output represents predicted probability of
    positive economic impact (trade volume growth) for each target city.
    """

    def __init__(self, n_targets: int = 29):
        super().__init__()
