...
We introduce five modifications, each addressing a distinct source of suboptimality:
...
\begin{enumerate}
    ...
    \item \textbf{Fixed-point NE search (FP-NE).} A meta-algorithm discovering Nash equilibria as fixed points of the smoothed best-response map $\mathrm{BR}_\tau$ (a Banach contraction for $\tau > M$), with Bayesian stopping via Good-Turing estimators. All selfish agents benefit from thorough search --- cooperation emerges from the topology of the fixed-point set (Appendix~\ref{app:fpne}).
\end{enumerate}

All five compose into the $\Omega$-gradient:
\begin{equation}
...
We extended the convergence framework of Giannou et al.~\cite{giannou2022} with five orthogonal modifications composing into the $\Omega$-gradient: evidence weighting ($\operatorname{HM}/\operatorname{AM}$ variance improvement), opponent shaping (basin enlargement $\mu \to \mu + \lambda\mu_H$), cooperative communication (self-knowledge bottleneck, coalition formation), sparse regularisation (blessing of dimensionality, $O(k \log d)$ scaling), and fixed-point NE search (Bayesian exploration of the equilibrium set via Banach contraction). All results are formalised in Lean~4 and validated experimentally on one-shot matrix games, fixed-point NE discovery, and iterated games adapted from Kim et al.~\cite{kim2021policygradientalgorithmlearning}. Applications to federated learning, RLHF alignment, and multi-agent debate demonstrate the framework's breadth (Appendix~\ref{app:applications}).
...
\begin{enumerate}
    ...
    \item \emph{FP-NE homotopy}: tracking QRE bifurcations as $\tau \to 0$ to map the full equilibrium correspondence; connections to catastrophe theory.
    \item \emph{Meta-learning + FP-NE}: integrating trajectory-level meta-optimisation (Meta-MAPG) with fixed-point search to escape defection equilibria in iterated games.
\end{enumerate}
...
\input{chapters/ch10-cooperative-communication}

\input{chapters/ch12-blessing-dimensionality}

\input{chapters/ch13-applications}

\input{chapters/ch14-fixed-point-ne}
...
\begin{itemize}
    ...
    \item \texttt{FixedPointNE.lean} --- BR$_\tau$ contraction, Banach uniqueness, exploration value bound.
\end{itemize}
